...
app.use(idempotencyMiddleware(3600)); 
import wallyV1Routes from "./src/api/wallyV1Routes";

const app = express();
const PORT = process.env.PORT || 3001;
app.use("/api/wallyv1", wallyV1Routes);
...
await db.run(
  ...
  ['paymaster-transfer', `Transferred ${toSend.toString()} from ${userAddress} to ${targetAddress} (leaving threshold ${threshold})`, txHash]
);

await db.run(
  `INSERT INTO activity_log (wallet_address, action, message, timestamp) VALUES (?, ?, ?, datetime('now'))`,
  [userAddress, "transfer", `Sent ${toSend} to ${destination}`]
);
    
// Emit to frontend via WebSocket or store for Activity Log endpoint
res.status(200).json({
  type: 'success',
